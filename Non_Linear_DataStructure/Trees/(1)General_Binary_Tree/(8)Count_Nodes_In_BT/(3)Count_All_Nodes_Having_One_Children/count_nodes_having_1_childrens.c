...
//Count number of nodes having Exactly one Children
...
/*
(head->left != NULL && head->right == NULL) || (head->left == NULL && head->right != NULL)
                  =
head->left != NULL ^ head->right != NULL (Called XOR Operator)
...
int main(){
  ...
  printf("\n\nPreorder Traversal: ");
  preorder(head);
  printf("\nNumber of Nodes Having Exactly 1 Childrens Are: %d", count(head));
